...
```
kubectl create secret generic kubernetes-nobleprog \
  --from-literal="mykey=mydata"
```

Print a hexdump of the `kubernetes-nobleprog` secret stored in etcd:
...
gcloud compute ssh controller-0 \
  ...
  /registry/secrets/default/kubernetes-nobleprog | hexdump -C"
```
...
```
gcloud compute firewall-rules create kubernetes-nobleprog-allow-nginx-service \
  --allow=tcp:${NODE_PORT} \
