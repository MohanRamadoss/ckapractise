...
```
docker run --rm -it --cap-add SYS_ADMIN --device /dev/fuse pschmitt/sshfs
```
...
Example:
* Copy to container: `docker cp foo.txt 72ca2488b353:/foo.txt`
* Copy from container: `docker cp 72ca2488b353:/foo.txt foo.txt`
* Export a container: `docker export red_panda > latest.tar` or `docker export --output="latest.tar" red_panda`
### Executing Commands
